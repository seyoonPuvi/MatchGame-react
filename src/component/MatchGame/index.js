...
const initialTab = tabsList[0].tabId
const randomImage = imagesList[0]

class MatchGame extends Component {
  state = {
    score: 0,
    timeElapsedInSeconds: 60,
    tabToShown: initialTab,
    randomImageToShown: randomImage,
    generatedImageIdList: [],
    isGameRunning: true,
  }
  ...
  onTimer = () => {
    const {timeElapsedInSeconds, isGameRunning, score} = this.state

    if (timeElapsedInSeconds > 0 || isGameRunning || score < 30) {
  ...
  updateTabToShown = id => {
    this.setState({tabToShown: id})
  }

  getTabList = () => {
    const {tabToShown} = this.state

    const updatedTabList = imagesList.filter(
      each => each.category === tabToShown,
    )

    return updatedTabList
  }

  generateNewRandomImageToShow = () => {
    const {generatedImageIdList} = this.state
    const updatedImageToShowList = imagesList.filter(
      each => !generatedImageIdList.includes(each.id),
    )
    const newImageIndex = Math.floor(
      Math.random() * updatedImageToShowList.length,
    )

    const newRandomImage = updatedImageToShowList[newImageIndex]

    this.setState({randomImageToShown: newRandomImage})
  }

  checkAndUpdateScore = id => {
    const {randomImageToShown} = this.state
    if (randomImageToShown.id === id) {
      this.generateNewRandomImageToShow()
      this.setState(prevState => ({
        score: prevState.score + 1,
        generatedImageIdList: [...prevState.generatedImageIdList, id],
        isGameRunning: true,
      }))
    } else {
      this.setState({isGameRunning: false})
  ...
  getScoreCard = () => {
    ...
    return (
      <div className="main-cont score-card-bg">
        ...
        <button type="button" className="reset" onClick={this.onReset}>
          <img
  ...
  getMainCont = filteredTabList => {
    const {randomImageToShown, tabToShown} = this.state
    const {imageUrl} = randomImageToShown
    return (
      <div className="main-cont">
        <img src={imageUrl} alt="match" className="random-img" />
        <div className="tabs-cont">
          <ul className="tab-category-cont">
            {tabsList.map(each => (
              <GameTab
                tabBtnDetails={each}
                updateTabToShown={this.updateTabToShown}
                tabToShown={tabToShown}
                key={each.tabId}
              />
            ))}
          </ul>
          <ul className="thumbnail-cont">
            {filteredTabList.map(each => (
              <Tabs
                ...
                key={each.id}
                checkAndUpdateScore={this.checkAndUpdateScore}
              />
            ))}
          </ul>
        </div>
  ...
  render() {
    const {score, isGameRunning, timeElapsedInSeconds} = this.state

    const filteredTabList = this.getTabList()
    const mainCont = isGameRunning
      ? this.getMainCont(filteredTabList)
      : this.getScoreCard()
    ...
    return (
      <div className="app-cont">
        <ul className="nav-bar">
          ...
          <li className="score-time-counter">
            <p className="score">
            ...
            <div className="timer">
              <img
                ...
                alt="timer"
              />
              <p className="count">{`${timeElapsedInSeconds} sec`}</p>
            </div>
          </li>
        </ul>
        {mainCont}
